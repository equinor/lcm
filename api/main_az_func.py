...
import logging
import json
from flask import CommandNotFound

import azure.functions as func
from azure.common import AzureMissingResourceHttpError
from ..Common import DatabaseOperations as db
from ..Common.Classes import Product, Mode
...
# The main function. This interprets the request type
# and calls the correct handler. On invalid or non-existent
# request input, an error message is returned.
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if request == BLEND_REQUEST:
        ...
        return func.HttpResponse(
            body=json.dumps({"Error": "Invalid request: " + str(request)}),
            status_code=400,
        )

    else:
        return func.HttpResponse(body="Error: No request", status_code=400)
...
def blendRequestHandler(req: func.HttpRequest):
    ...
    try:
        ...
        if percent_sum != 100:
            return func.HttpResponse(
                body=json.dumps({"Error": "Invalid percentages"}), status_code=400
            )

    except ValueError:
        return func.HttpResponse(
            body=json.dumps({"Error": "Missing or invalid products"}), status_code=400
        )
    except TypeError:
        return func.HttpResponse(
            body=json.dumps({"Error": "Missing or invalid products"}), status_code=400
...
def bridgeRequestHandler(req: func.HttpRequest):
    error_list = []
...
def productListRequestHandler(req: func.HttpRequest):
    ...
    try:
        ...
        return func.HttpResponse(
            body=json.dumps({"Error": "Invalid filters"}), status_code=400
        )
    except KeyError:
        return func.HttpResponse(
            body=json.dumps({"Error": "Invalid filters"}), status_code=400
    ...
    return func.HttpResponse(body=json.dumps(return_list))
...
def productRequestHandler(req: func.HttpRequest):
    ...
    try:
        ...
        return func.HttpResponse(
            body=json.dumps({"Error": "Invalid ID"}), status_code=400
        )
    except TypeError:
        return func.HttpResponse(
            body=json.dumps({"Error": "Invalid ID"}), status_code=400
