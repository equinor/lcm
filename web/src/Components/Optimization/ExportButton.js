      ...
      {visibleScrim && (
        <Scrim onClose={handleClose}>
          <Dialog>
            ...
            <div>
            <Wrapper>
                <Typography variant="body_short" bold>Optimal blend: </Typography>
                {(fetched && optimizationData && optimizationData.products && optimizationData.products.length !== 0) ? optimizationData.products.map((product) => 
                ((product.sacks !== 0) ?
                <div>
                ...
                {(fetched && optimizationData) ?
                  <div>
                  <Typography variant="body_short" bold>Total mass: </Typography>
                  <Typography variant="body_short">{getTotalMass()} kg</Typography>
                  <Typography variant="body_short" bold>Fit: </Typography>
            ...
            <Actions>
              {actionsChoice === 'buttons' ? (
                  <Button variant="ghost" onClick={() => setVisibleScrim(false)}>OK</Button>
              ) : (
